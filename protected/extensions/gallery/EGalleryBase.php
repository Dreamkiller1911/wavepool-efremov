...
class EGalleryBase extends CWidget {
	...
	/**
	 * @var string the name of the media
	 */
	public $name = 'Test media';
	...
	/**
	 * @var string the #ID of the media container
	 */
	public $id = 'media';
	...
	/**
	 * @var string the real server path to the image media
	 */
	protected $_realpath;
	...
	/**
	 * @var array the list of albums in the media
	 */
	protected $_albums = array();

	/**
	 * @var string the CSS file asset generated to include the #ID of the media
	 */
	protected $_cssFile;
	 ...
	 *
	 * Cleanup the {@see $this->path media path} and check that it's valid.
	...
	protected function setup(){
		...
		if($this->path != '' && file_exists($this->_realpath)):
			...
			Yii::log('Path to media is not specified or invalid: '.$this->path, 'info', 'app.extensions.EGallery');
			throw new CException('Path to media is not specified or invalid.');
		endif;
	}
	...
	/**
	 * Writes the array to a text file in /path/to/media/needsThumbs.txt for batch processing.
	...
	{
		$contents = '/*
 * This file is automatically generated. You should edit media.css.php instead.
 */

';

		if (is_file($folder.DIRECTORY_SEPARATOR.'media.css.php')) {
			ob_start();
			include $folder.DIRECTORY_SEPARATOR.'media.css.php';
			$contents .= ob_get_contents();
			ob_end_clean();
		}

		if($contents != file_get_contents($folder.DIRECTORY_SEPARATOR.'media.css') && is_writable($folder.DIRECTORY_SEPARATOR.'media.css'))
		{
			file_put_contents($folder.DIRECTORY_SEPARATOR.'media.css', $contents);
		}

		return $folder.DIRECTORY_SEPARATOR.'media.css';
	}
}
?>